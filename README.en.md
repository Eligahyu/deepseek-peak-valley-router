...
- **Your explicit choice wins**: any model you pick in the model selector is respected and left untouched (detected via changes to the `agent-default-model` setting)
...
### Option B: Composition plugin (permanent install, requires restart)

1. Copy this repository into the profile's shared node_modules:
...
2. Register it in the profile's patch layer (append to e.g. `~/.dsh/profiles/web/cordis.patch.yml`):

   ```yaml
   - insert:
       - id: deepseek-peak-valley-router
         name: deepseek-peak-valley-router
         # optional config: reference time zone (default Asia/Shanghai)
         # config:
         #   timeZone: 'Asia/Shanghai'
   ```

3. Restart `dsh web` (new rows are only loaded by the Loader after a restart).

> Dependencies: runs inside DeepSeek Harness; `@deepseek-ai/dsh-tools` is provided by the host (see peerDependencies in package.json).

## Behavior rules (priority, highest first)

1. **A model you explicitly selected — always wins.** Every model you pick in the selector is written to the `agent-default-model` setting; the plugin listens for that change and only respects it from then on. If the default model is already pro (not the factory-default flash) at plugin startup, it is treated as your historical choice.
...
- **Time zone conversion**: `lib/timezone.js` uses `Intl.DateTimeFormat` (IANA time zones, `hourCycle: 'h23'`) to convert the machine's wall clock to the reference zone — correct under any system time zone / DST; falls back to the `UTC+8` offset (Asia/Shanghai has no DST) when Intl is unavailable.
...
- **Your choice**: Harness's own session model-selection mechanism (`installModelSelection`) already guarantees that an explicit user selection wins; the plugin only auto-routes when there is no user selection, and carries its own respect check as a second layer.
...
- Run the tests: `npm test` (Node ≥ 20, `node --test`)
...
├── test/
│   └── timezone.test.js  # node --test cases (conversion, boundaries, cross-midnight)
├── package.json
├── README.md
└── LICENSE
```
