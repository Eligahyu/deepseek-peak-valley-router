 ...
 *
 * 行为规则(从高到低):
 *   1. 用户显式选择的模型(模型选择器保存的默认模型)→ 同一 provider 下
 *      直接路由到该模型,请求当前配置是什么都尊重用户选择;
 ...
 *
 * 安装方式见 README;本文件是 cordis 组合插件(profile 永久安装,支持
 * `dsh plugin add`),dynamic/host-code.js 是动态插件(cordis_define 粘贴)版本。
 */
...
/** ctx.logger 优先,缺失时回退 console(与动态版沙箱行为一致)。 */
 ...
 *   timeZone: IANA 参考时区,默认 'Asia/Shanghai'。无效值回退默认并告警。
 */
export default {
  name: 'deepseek-peak-valley-router',
  inject: ['tools'],
  apply(ctx, config = {}) {
    // 时区配置:校验一次;无效值回退默认时区,绝不让请求路径抛错。
    const rawTimeZone = typeof config.timeZone === 'string' && config.timeZone.length > 0
      ? config.timeZone
      : DEFAULT_TIME_ZONE
    let timeZone = DEFAULT_TIME_ZONE
    if (rawTimeZone !== DEFAULT_TIME_ZONE) {
      if (isValidTimeZone(rawTimeZone)) {
        timeZone = rawTimeZone
      } else {
        log(ctx, 'warn', `peak-valley: 配置的 timeZone "${rawTimeZone}" 无效,已回退到 ${DEFAULT_TIME_ZONE}`)
      }
    }
    ...
    // apply 时刻它可能尚未就绪,一次性 ctx.get() 存在竞态。首次路由请求时
    // 若默认模型已是 pro(非出厂默认 flash),视为用户先前显式选择。
    ...
    ctx.on('settings/updated', (ns, next) => {
      ...
      log(ctx, 'info', `peak-valley: 检测到用户显式模型选择 ${provider}/${model},后续请求尊重该选择(不再自动切换)`)
    })
    ...
    ctx.on('agent/request', async (_payload, next) => {
      ...
      if (userPicked !== undefined && userPicked.provider === config.provider) {
        ...
        log(ctx, 'info', `peak-valley: 尊重用户显式选择 ${userPicked.model}(请求配置 ${config.model})`)
      ...
      if (lastState !== label) {
        lastState = label
        log(ctx, 'info', `peak-valley: 进入${label}时段(${state.clock}),DeepSeek 模型路由 -> ${state.target}`)
      }
      log(ctx, 'info', `peak-valley: ${config.model} -> ${state.target} (${label}, ${state.clock})`)
