...
- **用户选择优先**:你在模型选择器里显式选择的模型会被**直接路由到**(同一 provider 下,无论请求当前配置是什么),通过监听 `agent-default-model` 设置的变更事件识别
...
- **健壮性**:非法 `timeZone` 配置自动回退默认时区并告警,绝不会让请求失败;启动兜底采用懒加载,与宿主服务加载顺序无关
...
### 方式 B:组合插件(官方 bundle 安装,永久生效,需要重启)

本包是标准 DSH bundle(声明了 `dsh.bundle` 与 `cordis.patch.yml`),用官方安装路径即可:
...
时区配置(可选,默认 `Asia/Shanghai`):在 profile 自己的补丁层覆盖即可:
...
> 依赖:运行在 DeepSeek Harness 内,`@deepseek-ai/dsh-tools` 由宿主提供(见 package.json 的 peerDependencies),经 launcher 维护的扁平模块回退自动解析,无需手动安装。

## 行为规则(优先级从高到低)

1. **用户显式选择的模型 —— 永远优先**。你在模型选择器里选的每个模型都会写入 `agent-default-model` 设置,插件监听该设置的变更事件;一旦检测到,同一 provider 下的所有请求都**直接路由到你选择的模型**(请求当前配置来自会话/子代理的冻结值时也不例外)。插件启动后首个请求若发现默认模型已是 pro(非出厂默认 flash),也会视为你的历史选择。
2. **自动路由(默认)**。未检测到显式选择时,按参考时区自动切换。
3. **不越界**。非 DeepSeek provider 或非 V4 双型号的请求原样放行。

## 工作原理

- **时区换算**:`lib/timezone.js` 用 `Intl.DateTimeFormat`(IANA 时区,`hourCycle: 'h23'`)把机器墙钟时间换算到参考时区,任何系统时区/夏令时下都正确;非法时区回退默认时区,Intl 不可用时回退 `UTC+8` 偏移(Asia/Shanghai 无夏令时)——请求路径上绝不抛错。
- **路由缝**:挂接 `agent/request` 瀑布事件——`next()` 产出冻结的 `LlmCallConfig`,返回替换对象即完成切换,循环会把切换记录进 `request/header`。
- **选择权**:用户显式选择由 Harness 自身的会话模型选择机制(`installModelSelection`)保证优先;插件在自身 respect 检查里**主动路由到用户选择**,双保险。
...
> 双版本差异:动态版额外暴露 `harness.handle('peak-valley-status')` Host 通道(供后续 Client UI 查询);组合版没有 Host 通道,状态查询通过对话级工具 `peakvalley_status` 完成。其余行为完全一致。

## 验证

- 运行测试:`npm test`(Node ≥ 20,`node --test`,21 个用例:时区纯逻辑 + 路由行为)
- 启动后看日志:进入高峰/空闲时打印 `peak-valley: 进入高峰时段(...),DeepSeek 模型路由 -> deepseek-v4-flash`;每次切换打印 `peak-valley: deepseek-v4-flash -> deepseek-v4-pro (...)`
...
├── cordis.patch.yml           # bundle patch(官方 dsh plugin add 安装入口)
├── lib/
...
├── test/
│   ├── timezone.test.js       # 时区换算、边界时刻、跨午夜
│   └── router.test.js         # 路由行为:自动路由/用户选择优先/时区容错/工具
├── package.json
├── CHANGELOG.md
├── README.md                  # 中文文档
